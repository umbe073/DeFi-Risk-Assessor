...
class DeFiRiskAssessor:
    ...
    def get_holder_data(self, token_address, chain):
        ...
        # Source 2: Breadcrumbs Risk Score
        try:
...
def process_token_batch(input_file="tokens.csv", output_file="risk_report.csv", json_output_file="risk_report.json"):
    ...
    def process_one(token, token_index, total_tokens):
        ...
        try:
            ...
            # Convert specific red flags to Yes/No format for CSV output
            ...
            summary = {
                ...
                "mica_non_compliant": mica_non_compliant,
...
def fetch_alchemy_compliance(token_address, chain):
    """Fetch compliance data from Alchemy API and calculate risk based on available data"""
    import os, requests
    ...
    try:
        # Try multiple Alchemy endpoints for comprehensive compliance analysis
        compliance_score = 0
        risk_factors = []
        ...
        try:
            ...
            if resp.status_code == 200:
                data = resp.json()
                if data.get('result') and data['result'] != '0x':
                    # Contract exists, check if it's verified
                    try:
                        # Try to get contract metadata
                        metadata_url = f"https://eth-mainnet.g.alchemy.com/v2/{api_key}"
                        metadata_payload = {
                            "jsonrpc": "2.0",
                        ...
                        metadata_resp = requests.post(metadata_url, json=metadata_payload, timeout=20)
                        if metadata_resp.status_code == 200:
                            ...
                            risk_factors.append("Contract verification unknown")
                    except:
                        risk_factors.append("Contract verification unknown")
                else:
        ...
        # 2. Token transfer analysis for suspicious patterns
        try:
            url = f"https://eth-mainnet.g.alchemy.com/v2/{api_key}"
            payload = {
                "jsonrpc": "2.0",
            ...
            if resp.status_code == 200:
                ...
                if transfers:
                    ...
                    if exchange_transfers > len(transfers) * 0.8:
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if metadata:
                    ...
                    suspicious_keywords = ['scam', 'fake', 'test', 'rug', 'honeypot']
                    ...
                    risk_factors.append("No token metadata")
                    compliance_score -= 1
        except Exception as e:
        ...
        if compliance_score >= 2:
            return {"summary": f"Alchemy compliant (score: {compliance_score})", "score_delta": -2}
        elif compliance_score >= 0:
            return {"summary": f"Alchemy neutral (score: {compliance_score})", "score_delta": 0}
        else:
            risk_summary = "; ".join(risk_factors[:3])  # Limit to first 3 risk factors
            return {"summary": f"Alchemy non-compliant: {risk_summary}", "score_delta": 2}
