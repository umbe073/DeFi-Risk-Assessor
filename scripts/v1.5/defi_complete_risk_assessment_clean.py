...
def fetch_data_with_cache_fallback(token_address, fetch_function):
    """Fetch data with cache fallback strategy"""
    ...
    # If no cache or cache is old, fetch real-time data
...
# Create timestamped Excel report path
timestamp = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
EXCEL_REPORT_PATH = os.path.join(DATA_DIR, f'DeFi Tokens Risk Assessment Results_{timestamp}.xlsx')
...
def write_failed_endpoints_summary():
    ...
    try:
        with open(SUMMARY_TXT, 'a') as f:
            ...
            f.write("="*50 + "\n")
            f.write(f"Total failed endpoints: {len(failed_api_endpoints)}\n\n")
            ...
            if failed_api_endpoints:
                for endpoint in failed_api_endpoints:
                    ...
                    f.write(f"Timestamp: {endpoint['timestamp']}\n")
                    f.write("-" * 30 + "\n")
            else:
...
- COINAPI_API_KEY: For comprehensive market data and historical analysis (optional)
"""
...
def check_api_keys():
    ...
    if COINAPI_API_KEY:
        print("  ✅ COINAPI API Key: Available")
    else:
        print("  ❌ COINAPI API Key: Missing")
    ...
    print("=" * 40)
    print(f"📊 Summary: {sum([bool(COINGECKO_API_KEY), bool(COINMARKETCAP_API_KEY), bool(COINAPI_API_KEY), bool(ETHERSCAN_API_KEY), bool(ETHPLORER_API_KEY), bool(BITQUERY_API_KEY), bool(MORALIS_API_KEY), bool(CERTIK_API_KEY), bool(SCORECHAIN_API_KEY), bool(TRM_LABS_API_KEY), bool(OPENSANCTIONS_API_KEY), bool(LUKKA_API_KEY), bool(ALCHEMY_API_KEY), bool(DEFISAFETY_API_KEY), bool(TWITTER_API_KEY), bool(TELEGRAM_BOT_TOKEN), bool(DISCORD_BOT_TOKEN), bool(REDDIT_CLIENT_ID), bool(ZAPPER_API_KEY), bool(DEBANK_API_KEY), bool(INCH_API_KEY), bool(DUNE_API_KEY), bool(SANTIMENT_API_KEY)])}/23 APIs available")
    print()
...
def fetch_coinapi_market_data(symbol, asset_id=None):
    """Fetch comprehensive market data from COINAPI"""
    if not COINAPI_API_KEY:
        print(f"    ⚠️  COINAPI key missing")
        return None
    
    try:
        # First, get asset ID if not provided
        if not asset_id:
            ...
            if response and response.status_code == 200:
                ...
                for asset in assets:
                    if asset.get('asset_id') == symbol.upper():
                        asset_id = asset.get('asset_id')
                        break
        
        if not asset_id:
            print(f"    ⚠️  COINAPI: Asset ID not found for {symbol}")
            return None
        
        # Get current market data
        url = f"https://rest.coinapi.io/v1/exchangerate/{asset_id}/USD"
        headers = {'X-CoinAPI-Key': COINAPI_API_KEY}
        response = robust_request('GET', url, headers=headers, timeout=15)
        ...
        if response and response.status_code == 200:
            ...
            url = f"https://rest.coinapi.io/v1/ohlcv/{asset_id}/USD/history"
            params = {
                'period_id': '1DAY',
            ...
            # Get order book data for liquidity estimation
            url = f"https://rest.coinapi.io/v1/orderbook/{asset_id}/USD"
            response = robust_request('GET', url, headers=headers, timeout=15)
            liquidity = 0
            if response and response.status_code == 200:
            ...
            print(f"    ❌ COINAPI failed: {response.status_code if response else 'No response'}")
    except Exception as e:
        print(f"    ❌ COINAPI error: {e}")
    return None

def fetch_coinapi_historical_data(symbol, days=30):
    """Fetch historical price and volume data from COINAPI"""
    if not COINAPI_API_KEY:
        print(f"    ⚠️  COINAPI key missing")
        return None
    ...
    try:
        ...
        url = f"https://rest.coinapi.io/v1/ohlcv/{symbol.upper()}/USD/history"
        params = {
            'period_id': '1DAY',
            'time_start': start_time.isoformat() + 'Z',
            'time_end': end_time.isoformat() + 'Z',
        ...
        if response and response.status_code == 200:
            ...
            if data:
                ...
                prices = [day['price_close'] for day in data]
                volumes = [day['volume_traded'] for day in data]
                ...
                return {
                    ...
                    'price_change_30d': (prices[-1] - prices[0]) / prices[0] if prices else 0,
                    'volume_trend': 'increasing' if len(volumes) > 1 and volumes[-1] > volumes[0] else 'decreasing',
                    'data_source': 'COINAPI'
                }
        else:
            print(f"    ❌ COINAPI historical data failed: {response.status_code if response else 'No response'}")
    except Exception as e:
        print(f"    ❌ COINAPI historical data error: {e}")
    return None
...
def fetch_breadcrumbs_risk_score(address):
    """Fetch sanctions data from Breadcrumbs API (limited to sanctions endpoint)"""
    if not BREADCRUMBS_API_KEY:
        print(f"    ⚠️  Breadcrumbs API key missing")
        return None
    
    try:
        # Use sanctions endpoint only (as per user's API limitations)
        url = f"https://api.breadcrumbs.one/sanctions/address?chain=ETH&address={address}"
        ...
        if response and response.status_code == 200:
            ...
            # Extract sanctions information and convert to risk score
            sanctions_status = data.get('sanctions_status', 'unknown')
            risk_score = 0
            
            # Convert sanctions status to risk score
            if sanctions_status == 'sanctioned':
                risk_score = 10  # High risk
            elif sanctions_status == 'clean':
                risk_score = 1   # Low risk
            else:
                risk_score = 5   # Medium risk (unknown)
            ...
            print(f"      ⚠️  Breadcrumbs sanctions API limited (403)")
            return {
                'risk_score': 0,
                'risk_level': 'Sanctions Check',
                'compliance_status': 'Sanctions API Limited (403)'
            }
        else:
            print(f"      ❌ Breadcrumbs sanctions API failed: {response.status_code if response else 'No response'}")
            return None
    except Exception as e:
...
def fetch_breadcrumbs_token_info(address):
    """Fetch sanctions info from Breadcrumbs API (limited to sanctions endpoint)"""
    if not BREADCRUMBS_API_KEY:
        print(f"    ⚠️  Breadcrumbs API key missing")
        return None
...
def fetch_zapper_portfolio_data(address):
    ...
    payload = {
        ...
        'variables': {
            'addresses': [address],
            'networks': ['ETHEREUM_MAINNET', 'POLYGON_MAINNET', 'B2_MAINNET']
        }
    }
    
    try:
        response = robust_request('POST', url, headers=headers, json=payload, timeout=30)
...
def fetch_moralis_token_metadata(address, chain="eth"):
    """Fetch token metadata from Moralis with improved error handling and multiple endpoints"""
    if not MORALIS_API_KEY:
        print(f"    ⚠️  Moralis API key missing")
        return None
    
    try:
        # Try multiple endpoints with different chain parameters
        endpoints = [
            f"https://deep-index.moralis.io/api/v2/erc20/{address}?chain=eth",
            f"https://deep-index.moralis.io/api/v2/erc20/{address}?chain=bsc",
            f"https://deep-index.moralis.io/api/v2/erc20/{address}?chain=polygon",
            f"https://deep-index.moralis.io/api/v2/erc20/{address}",
            f"https://deep-index.moralis.io/api/v2/erc20/{address}?chain=mainnet"
        ]
        ...
        for i, url in enumerate(endpoints):
            try:
                ...
                if response and response.status_code == 200:
                    ...
                    print(f"      ⚠️  Moralis token not found for address {address[:8]}... (endpoint {i+1})")
                    continue  # Try next endpoint
                else:
                    print(f"      ⚠️  Moralis API endpoint {i+1} failed: {response.status_code if response else 'No response'}")
                    continue  # Try next endpoint
                    
            except Exception as e:
                print(f"      ⚠️  Moralis endpoint {i+1} error: {e}")
                continue  # Try next endpoint
        
        # If all endpoints failed, try alternative approach
        try:
            # Try with different API version
            alt_url = f"https://deep-index.moralis.io/api/v1/erc20/{address}"
            response = robust_request('GET', alt_url, headers=headers, timeout=15)
            
            if response and response.status_code == 200:
                data = response.json()
                print(f"      ✅ Moralis token metadata found (v1 API)")
                return data
        except Exception:
            pass
...
def robust_request(method, url, **kwargs):
    """Make a robust HTTP request with retry logic and optimized rate limiting"""
    max_retries = 2  # Reduced from 3 to 2 for faster processing
    base_delay = 0.2  # Reduced from 0.5 to 0.2 for faster processing
    
    for attempt in range(max_retries):
        try:
            # Add minimal delay only on retries
            if attempt > 0:
                time.sleep(base_delay * (1.5 ** attempt))  # Reduced exponential backoff
            
            response = requests.request(method, url, **kwargs)
            
            # Handle rate limiting specifically
            if response.status_code == 429:
                # Service-specific rate limit delays (optimized for faster processing)
                service_delays = {
                    'api.coingecko.com': 1,  # CoinGecko free tier (further reduced)
                    'pro-api.coinmarketcap.com': 5,  # CMC (further reduced)
                    'api.etherscan.io': 1,  # Etherscan (further reduced)
                    'deep-index.moralis.io': 2,  # Moralis (further reduced)
                    'api.covalenthq.com': 3,  # Covalent (further reduced)
                    'api.twitter.com': 8,  # Twitter (further reduced)
                    'api.santiment.net': 4,  # Santiment (further reduced)
                    'default': 2  # Default further reduced
                }
                
                domain = url.split('/')[2] if '/' in url else 'default'
                delay = service_delays.get(domain, service_delays['default'])
                delay += (attempt * 0.5)  # Minimal exponential backoff for faster processing
                
                print(f"    ⚠️  Rate limit hit for {domain}, waiting {delay}s...")
                
                # Skip fallback in standalone function - just wait for rate limit
                # Fallback data only available in class methods
                
                time.sleep(delay)
                continue
            
            # Handle specific HTTP errors with fallbacks
            if response.status_code >= 400:
                # Log error (fallback data not available in standalone function)
                if response.status_code in [401, 403, 404, 429]:
                    print(f"    ❌ HTTP {response.status_code} error for {url}")
                
                # Log errors more selectively (reduce noise)
                if response.status_code not in [404]:  # 404s are often normal for tokens
                    print(f"❌ API Error: {response.status_code} {response.reason} for {url}")
                    if kwargs.get('params'):
                        print(f"   Params: {kwargs.get('params', {})}")
                    
                    # Only log response content for unexpected errors
                    if response.status_code not in [401, 403, 404, 429] and response.text:
                        print(f"   Response: {response.text[:200]}...")
                
                # Log to failed endpoints summary only for non-routine errors
            ...
            if attempt < max_retries - 1:
                delay = base_delay * (1.5 ** attempt) + random.uniform(0, 0.2)  # Faster retry delays
...
class DeFiRiskAssessor:
    ...
    def __init__(self):
        ...
        try:
            self.available_keys = check_api_keys() or {}
            print(f"  ✅ API keys checked: {len(self.available_keys)} available")
        except Exception as e:
        ...
        # Initialize Web3
        try:
            print(f"  🔧 Initializing Web3 with INFURA_API_KEY: {'Set' if INFURA_API_KEY else 'Not set'}")
            if INFURA_API_KEY:
        ...
        # Initialize sentiment scoring helpers
    ...
    def robust_request(self, method, url, **kwargs):
        """Class method wrapper for robust HTTP requests"""
        # Call the global function with correct parameter order
    ...
    def _detect_stablecoin(self, token_address, symbol, market_data):
        """Detect if a token is a stablecoin based on address, symbol, and market data"""
        ...
        stablecoin_addresses = {
            ...
            "0x8e870d67f660d95d5be530380d0ec0bd388289e1": "PAXG",     # PAX Gold
            "0x7a58c0be72be218b41c608b7fe7c5bb630736c71": "PAX",      # Paxos Standard
            "0x0000000000085d4780b73119b644ae5ecd22b376": "TUSD",     # TrueUSD
        }
        
        # Check by address
        if token_address_lower in stablecoin_addresses:
            return True
        
        # Check by symbol (case-insensitive)
        stablecoin_symbols = {'usdt', 'usdc', 'dai', 'busd', 'frax', 'lusd', 'paxg', 'pax', 'tusd', 'gusd', 'husd', 'susd'}
        if symbol and symbol.lower() in stablecoin_symbols:
            return True
        
        # Check by price stability (if price is very close to $1)
        try:
            coingecko_data = market_data.get('coingecko', {}).get('market_data', {})
            current_price = coingecko_data.get('current_price', {}).get('usd', 0)
            if 0.95 <= current_price <= 1.05:  # Price within 5% of $1
                return True
        except:
            pass
    ...
    def get_contract_verification_status(self, token_address, chain):
        ...
        try:
            ...
            response = self.robust_request('GET', url, params=params, timeout=15)
        ...
        try:
            ...
            if response and response.status_code == 200:
                ...
                if data.get('status') == '1':
                    ...
                    holder_data['total_holders'] = len(unique_addresses)
    ...
    def get_liquidity_data(self, token_address, chain):
        ...
        if INCH_API_KEY:
            try:
                ...
                params = {
                    'src': token_address,
                    'dst': '0x6B175474E89094C44Da98b954EedeAC495271d0F',  # Use DAI as destination for liquidity check
                    'amount': '1000000000000000000'  # 1 token
                }
                response = self.robust_request('GET', url, headers=headers, params=params)
                if response and response.status_code == 200:
        ...
        try:
            ...
            response = self.robust_request('GET', url)
            if response and response.status_code == 200:
                data = response.json()
                if data.get('coins'):
    ...
    def fetch_bitquery_data(self, token_address, chain):
        """Fetch comprehensive data from Bitquery including token info, transfers, and DEX data"""
        if not BITQUERY_API_KEY:
            return {}
        
        try:
            print(f"  🔍 Fetching comprehensive BitQuery data for {token_address[:8]}...")
            
            url = "https://graphql.bitquery.io"
            headers = {'X-API-KEY': BITQUERY_API_KEY, 'Content-Type': 'application/json'}
            all_data = {}
            
            # 1. Token and Contract Information
            token_query = """
            ...
            response = self.robust_request('POST', url, headers=headers, json={
                'query': token_query,
            ...
            if response and response.status_code == 200:
                ...
                if token_data.get('data', {}).get('ethereum', {}).get('address'):
                    all_data['token_info'] = token_data['data']['ethereum']['address']
                    print(f"    ✅ BitQuery token info found")
            ...
            query ($address: String!) {
                ethereum {
                    dexTrades(
                        options: {limit: 100}
                        filter: {baseCurrency: {is: $address}}
                    ) {
                        count
                        tradeAmount(in: USD)
                        tradeAmount(in: USD, calculate: volume)
                        trades: count
                        currencies {
                            baseCurrency {
                                symbol
            ...
            if response and response.status_code == 200:
                ...
                if dex_data.get('data', {}).get('ethereum', {}).get('dexTrades'):
                    all_data['dex_trading'] = dex_data['data']['ethereum']['dexTrades']
                    print(f"    ✅ BitQuery DEX trading data found")
            
            # 3. Liquidity Pool Data
            liquidity_query = """
            query ($address: String!) {
                ethereum {
                    liquidityPools(
            ...
            if response and response.status_code == 200:
                ...
                if liquidity_data.get('data', {}).get('ethereum', {}).get('liquidityPools'):
                    all_data['liquidity_pools'] = liquidity_data['data']['ethereum']['liquidityPools']
                    print(f"    ✅ BitQuery liquidity pool data found")
            ...
            if response and response.status_code == 200:
                ...
                if holders_data.get('data', {}).get('ethereum', {}).get('transfers'):
                    all_data['holder_analysis'] = holders_data['data']['ethereum']['transfers']
                    print(f"    ✅ BitQuery holder analysis found")
            
            if all_data:
                print(f"    ✅ BitQuery comprehensive data collected")
                return all_data
            else:
                print(f"    ⚠️  No BitQuery data found")
                return {}
    ...
    def fetch_market_data(self, token_address, chain):
        ...
        if cmc_api_key:
            try:
                ...
                map_data: Optional[Dict[str, Any]] = cached if isinstance(cached, dict) else None
                if map_data is None:
        ...
        # COINAPI data integration
        try:
            token_symbol = self._get_token_symbol_from_address(token_address)
            if token_symbol and COINAPI_API_KEY:
                print(f"    📊 Fetching COINAPI data for {token_symbol}...")
                
                # Get current market data
                coinapi_data = fetch_coinapi_market_data(token_symbol)
                if coinapi_data:
                    data['coinapi'] = {
                        'market_data': {
                            'current_price': {'usd': coinapi_data.get('price_usd', 0)},
                            'total_volume': {'usd': coinapi_data.get('volume_24h', 0)},
                            'liquidity': coinapi_data.get('liquidity', 0)
                        },
                        'asset_id': coinapi_data.get('asset_id'),
                        'data_source': 'COINAPI'
                    }
                    print(f"    ✅ COINAPI data found")
                
                # Get historical data for volatility analysis
                historical_data = fetch_coinapi_historical_data(token_symbol, days=30)
                if historical_data:
                    data['coinapi']['historical_analysis'] = {
                        'price_volatility': historical_data.get('price_volatility', 0),
                        'avg_volume': historical_data.get('avg_volume', 0),
                        'price_change_30d': historical_data.get('price_change_30d', 0),
                        'volume_trend': historical_data.get('volume_trend', 'stable'),
                        'data_points': len(historical_data.get('historical_data', []))
                    }
                    print(f"    ✅ COINAPI historical analysis completed")
            else:
                print(f"    ⚠️  COINAPI: No symbol found or API key missing")
    ...
    def _get_token_symbol_from_address(self, token_address):
        ...
        try:
            if self.current_chain == 'eth' and ETHERSCAN_API_KEY:
                ...
                response = self.robust_request('GET', etherscan_url, params=params, timeout=15)
                if response and response.status_code == 200:
                    data = response.json()
                    if data.get('status') == '1':
        ...
        try:
            if self.current_chain in self.CHAIN_CONFIG:
                chain_config = self.CHAIN_CONFIG[self.current_chain]
            ...
            if os.path.exists(cmc_map_file):
                with open(cmc_map_file, 'r') as f:
                    ...
                    for token_data in cmc_data.get('data', []):
                        if token_data.get('platform', {}).get('token_address', '').lower() == token_address.lower():
    ...
    def _get_symbol_from_coingecko(self, token_address):
        ...
        try:
            # CoinGecko's contract info endpoint
            url = f"https://api.coingecko.com/api/v3/coins/ethereum/contract/{token_address.lower()}"
            
            headers = {}
            if COINGECKO_API_KEY:
                headers['x-cg-pro-api-key'] = COINGECKO_API_KEY
            
            response = self.robust_request('GET', url, timeout=10, headers=headers)
            if response and response.status_code == 200:
                data = response.json()
    ...
    def _get_symbol_from_coinmarketcap(self, token_address):
        ...
        try:
            ...
            response = self.robust_request('GET', url, timeout=10, headers=headers, params=params)
            if response and response.status_code == 200:
                data = response.json()
    ...
    def _save_symbol_to_cache(self, token_address, symbol, source):
        ...
        try:
            ...
            cache_data['symbols'][token_address.lower()] = {
                'symbol': symbol.upper(),
                'source': source,
                'timestamp': time.time(),
    ...
    def _load_symbol_from_cache(self, token_address):
        ...
        try:
            ...
            symbol_info = cache_data.get('symbols', {}).get(token_address.lower())
            if symbol_info:
    ...
    def _log_symbol_resolution_debug(self, token_address):
        ...
        try:
            debug_info = {
                ...
                'chain_config_available': token_address in getattr(self, 'CHAIN_CONFIG', {}),
    ...
    def fetch_enhanced_data(self, token_address, chain):
        ...
        try:
            ...
            inch_data = fetch_1inch_comprehensive_data(token_address)
        ...
        try:
            ...
            # Alchemy
            alchemy = fetch_alchemy_comprehensive_data(token_address, chain)
            if alchemy:
        ...
        try:
            ...
            if thegraph_data and not thegraph_data.get('placeholder'):
                ...
                print(f"      ⚠️  No The Graph data available")
        except Exception as e:
        ...
        try:
            ...
            # Add CoinDesk RSS data
            try:
                coindesk_data = self.fetch_coindesk_social_data(getattr(self, 'current_symbol', ''), getattr(self, 'current_token_name', ''))
                if coindesk_data:
                    social_data['coindesk'] = coindesk_data
                    enhanced_data['coindesk'] = coindesk_data
                    print(f"      ✅ CoinDesk RSS data found")
            except Exception as e:
                print(f"      ❌ CoinDesk error: {e}")
            
            # Add The Block data
            try:
                theblock_data = self.fetch_theblock_social_data(getattr(self, 'current_symbol', ''), getattr(self, 'current_token_name', ''))
                if theblock_data:
                    social_data['theblock'] = theblock_data
                    enhanced_data['theblock'] = theblock_data
                    print(f"      ✅ The Block data found")
            except Exception as e:
                print(f"      ❌ The Block error: {e}")
            
            # Add Decrypt RSS data
            try:
                decrypt_data = self.fetch_decrypt_social_data(getattr(self, 'current_symbol', ''), getattr(self, 'current_token_name', ''))
                if decrypt_data:
                    social_data['decrypt'] = decrypt_data
                    enhanced_data['decrypt'] = decrypt_data
                    print(f"      ✅ Decrypt RSS data found")
            except Exception as e:
            ...
            if social_data:
                ...
                print(f"      ✅ Social data found: {len(social_data)} sources")
            else:
    ...
    def assess_token(self, token_address, chain="eth", progress_callback=None, token_index=0, total_tokens=1):
        ...
        try:
            ...
            if cached_data_available:
                print(f"  ⚡ Using recent cached data (less than 1 hour old) to avoid rate limits...")
                # Use cached data for scoring, skip real-time API calls
                ...
                red_flags = cached_data_available.get('red_flags', [])
                print(f"  ✅ Using cached data to avoid rate limits")
            else:
    ...
    def _check_recent_cached_data(self, token_address, chain):
        """Check if recent cached data exists (less than 1 hour old) - ONLY real data, no estimates"""
        try:
            # Check cache file for recent data
            ...
            if os.path.exists(cache_file):
                ...
                if cache_age < 3600:  # Less than 1 hour old
                    with open(cache_file, 'rb') as f:
                        ...
                        if token_address in cache_data.get('data', {}):
                            cached_data = cache_data['data'][token_address]
                            # Check if cached data has all required fields and contains real data
                            required_fields = ['onchain_data', 'market_data', 'enhanced_data']
                            if all(field in cached_data for field in required_fields):
                                # Verify this is real data, not estimated
                                market_data = cached_data.get('market_data', {})
                                if market_data and any([
                                    market_data.get('coingecko'),
                                    market_data.get('cmc'),
                                    market_data.get('coinpaprika')
                                ]):
                                    print(f"  📦 Found recent cached data (age: {cache_age/60:.1f} minutes)")
                                    return cached_data
                                else:
                                    print(f"  ⚠️  Cached data exists but contains no real market data")
            
            # Check webhook-updated cache file
            webhook_cache_file = os.path.join(DATA_DIR, 'webhook_cache.json')
            if os.path.exists(webhook_cache_file):
                webhook_age = time.time() - os.path.getmtime(webhook_cache_file)
                if webhook_age < 3600:  # Less than 1 hour old
                    with open(webhook_cache_file, 'r') as f:
                        webhook_data = json.load(f)
                        token_data = webhook_data.get('token_data', {}).get(token_address)
                        if token_data and 'last_updated' in token_data:
                            # Check if webhook data is recent enough
                            last_updated = datetime.datetime.fromisoformat(token_data['last_updated'])
                            if (datetime.datetime.now() - last_updated).total_seconds() < 3600:
                                ...
                                if market_data and any([
                                    ...
                                    print(f"  📦 Found recent webhook cache data (age: {(datetime.datetime.now() - last_updated).total_seconds()/60:.1f} minutes)")
                                    return token_data
                                else:
                                    print(f"  ⚠️  Webhook cache exists but contains no real market data")
        ...
        try:
            ...
            for component in self.WEIGHTS.keys():
                try:
                    ...
                    component_scores[component] = 7.0  # Default to medium-high risk on error
            ...
            for component, weight in self.WEIGHTS.items():
                # Normalize component score to 0-15 range for better distribution
                normalized_score = (component_scores[component] - 1) * 1.67  # Convert 1-10 to 0-15
                component_contribution = normalized_score * weight * 10  # Scale to 0-150 range
                ...
                print(f"    {component}: {component_scores[component]:.2f} → {component_contribution:.2f} (weight: {weight})")
    ...
    def score_whitepaper_quality(self, risk_report):
        ...
        try:
            # Basic scoring for now - can be enhanced with actual whitepaper analysis
            score = 5  # Base score
            ...
            if coingecko_data.get('links', {}).get('homepage'):
                score += 1
            if coingecko_data.get('links', {}).get('whitepaper'):
                score += 2
            if coingecko_data.get('description', {}).get('en'):
                score += 1
    ...
    def score_roadmap_adherence(self, risk_report):
        ...
        try:
            # Basic scoring - can be enhanced with actual roadmap analysis
            score = 5  # Base score
            
            # Check for project activity indicators
            market_data = risk_report.get('market', {})
            coingecko_data = market_data.get('coingecko', {})
            
            if coingecko_data.get('community_data', {}).get('twitter_followers', 0) > 10000:
                score += 1
            if coingecko_data.get('developer_data', {}).get('forks', 0) > 0:
                score += 1
    ...
    def score_team_expertise(self, risk_report):
        ...
        try:
            # Basic scoring - can be enhanced with actual team analysis
            score = 5  # Base score
            ...
            if coingecko_data.get('links', {}).get('repos_url'):
                score += 1
            if coingecko_data.get('links', {}).get('homepage'):
                score += 1
    ...
    def score_management_strategy(self, risk_report):
        ...
        try:
            # Basic scoring - can be enhanced with actual strategy analysis
            score = 5  # Base score
            ...
            # Extract market cap with type safety
            market_cap_raw = coingecko_data.get('market_data', {}).get('market_cap', {}).get('usd', 0)
            try:
                market_cap = float(market_cap_raw) if isinstance(market_cap_raw, (int, float, str)) else 0.0
            except (ValueError, TypeError):
                market_cap = 0.0
            
            # Extract volume with type safety  
            volume_raw = coingecko_data.get('market_data', {}).get('total_volume', {}).get('usd', 0)
            try:
                volume = float(volume_raw) if isinstance(volume_raw, (int, float, str)) else 0.0
            except (ValueError, TypeError):
                volume = 0.0
    ...
    def score_aml_data(self, risk_report, token_address, chain):
        ...
        try:
            ...
            # Check for real data from Scorechain, TRM Labs, Lukka
            if SCORECHAIN_API_KEY and compliance_data.get('scorechain'):
                scorechain_data = compliance_data['scorechain']
                if not scorechain_data.get('placeholder', False):
                    score += scorechain_data.get('aml_score', 5) - 5
                    has_real_aml_data = True
            
            if TRM_LABS_API_KEY and compliance_data.get('trm_labs'):
                trm_data = compliance_data['trm_labs']
                if not trm_data.get('placeholder', False):
                    score += trm_data.get('aml_score', 5) - 5
                    has_real_aml_data = True
            
            # If no real AML data available, mark as placeholder
            if not has_real_aml_data:
                print(f"    ⚠️  AML scoring using basic indicators only (no API keys)")
    ...
    def score_compliance_data(self, risk_report, token_address, chain):
        ...
        try:
            ...
            # Check for real data from CertiK, DeFiSafety, Lukka
            if CERTIK_API_KEY and compliance_data.get('certik'):
                certik_data = compliance_data['certik']
                if not certik_data.get('placeholder', False):
                    score += certik_data.get('security_score', 5) - 5
                    has_real_compliance_data = True
            
            if DEFISAFETY_API_KEY and compliance_data.get('defisafety'):
                defi_data = compliance_data['defisafety']
                if not defi_data.get('placeholder', False):
                    score += defi_data.get('safety_score', 5) - 5
                    has_real_compliance_data = True
            
            if LUKKA_API_KEY and compliance_data.get('lukka'):
                lukka_data = compliance_data['lukka']
                if not lukka_data.get('placeholder', False):
                    score += lukka_data.get('compliance_score', 5) - 5
                    has_real_compliance_data = True
            
            # If no real compliance data available, mark as placeholder
            if not has_real_compliance_data:
                print(f"    ⚠️  Compliance scoring using basic indicators only (no API keys)")
                # Use basic onchain indicators
                onchain_data = risk_report.get('onchain', {})
                if onchain_data.get('contract_verified') == 'verified':
                    score += 1
    ...
    def score_market_dynamics(self, risk_report):
        ...
        try:
            ...
            # COINAPI data integration (weight: 15%)
            coinapi_data = market_data.get('coinapi', {})
            if coinapi_data:
                # Price volatility analysis from historical data
                historical_analysis = coinapi_data.get('historical_analysis', {})
                price_volatility = self.safe_float(historical_analysis.get('price_volatility', 0))
                
                if price_volatility > 0.5:  # High volatility (>50%)
                    score -= 1.0
                elif price_volatility > 0.2:  # Medium volatility (20-50%)
                    score -= 0.5
                elif price_volatility < 0.1:  # Low volatility (<10%)
                    score += 0.5
                
                # Volume trend analysis
                volume_trend = historical_analysis.get('volume_trend', 'stable')
                if volume_trend == 'increasing':
                    score += 0.5
                elif volume_trend == 'decreasing':
                    score -= 0.5
                
                # 30-day price change analysis
                price_change_30d = self.safe_float(historical_analysis.get('price_change_30d', 0))
                if abs(price_change_30d) > 0.5:  # Extreme 30-day change (>50%)
                    score -= 0.5
                elif abs(price_change_30d) < 0.1:  # Stable 30-day change (<10%)
                    score += 0.3
                
                # Order book liquidity analysis
                coinapi_market = coinapi_data.get('market_data', {})
                orderbook_liquidity = self.safe_float(coinapi_market.get('liquidity', 0))
                if orderbook_liquidity > 1_000_000:  # High order book liquidity
                    score += 0.5
                elif orderbook_liquidity < 100_000:  # Low order book liquidity
                    score -= 0.3
                
                print(f"      📊 COINAPI analysis: Volatility={price_volatility:.3f}, Trend={volume_trend}, 30d_change={price_change_30d:.2%}, Liquidity=${orderbook_liquidity:,.0f}")
    ...
    def score_marketing_demand(self, risk_report):
        ...
        try:
            ...
            if community_data.get('twitter_followers', 0) > 10000:
                score += 1
            if community_data.get('reddit_subscribers', 0) > 1000:
                score += 1
    ...
    def score_dev_activity(self, risk_report):
        ...
        try:
            ...
            if dev_data.get('forks', 0) > 0:
                score += 1
            if dev_data.get('stars', 0) > 10:
                score += 1
    ...
    def score_social_data(self, risk_report, token_address, symbol):
        ...
        try:
            ...
            # Check CoinDesk data
            if social_data.get('coindesk') and social_data['coindesk'].get('mention_count', 0) > 0:
                score += 1
            
            # Check The Block data
            if social_data.get('theblock') and social_data['theblock'].get('article_count', 0) > 0:
                score += 1
            
            # Check Decrypt data
            if social_data.get('decrypt') and social_data['decrypt'].get('mention_count', 0) > 0:
                score += 1
            
            # Check Bitcointalk data
            if social_data.get('bitcointalk') and social_data['bitcointalk'].get('sentiment_score', 0) != 0:
                score += 1
            
            # Check Cointelegraph data
            if social_data.get('cointelegraph') and social_data['cointelegraph'].get('article_count', 0) > 0:
                score += 1
            
            # Additional scoring based on sentiment quality
            total_sources = len([k for k, v in social_data.items() if v and not isinstance(v, dict) or (isinstance(v, dict) and any(v.values()))])
            if total_sources >= 5:
                score += 0.5  # Bonus for comprehensive coverage
            elif total_sources >= 3:
                score += 0.2  # Small bonus for good coverage
    ...
    def fetch_twitter_social_data(self, token_symbol, token_name):
        """Fetch Twitter social data with improved authentication and multiple endpoints"""
        try:
            ...
            # Method 1: Bearer token format
            if not TWITTER_API_KEY.startswith('Bearer ') and len(TWITTER_API_KEY) > 50:
                auth_methods.append(f"Bearer {TWITTER_API_KEY}")
            else:
                auth_methods.append(TWITTER_API_KEY)
            
            # Method 2: OAuth2 format (if different)
            if len(TWITTER_API_KEY) > 50:
                auth_methods.append(f"OAuth {TWITTER_API_KEY}")
            
            # Method 3: Direct token
            auth_methods.append(TWITTER_API_KEY)
            
            queries = [f'"{token_symbol}"', f'"{token_name}"', f'#{token_symbol}', f'#{token_name}']
                ...
                for i, query in enumerate(limited_queries):
                    try:
                        ...
                        if response and response.status_code == 200:
                            ...
                            print(f"    ⚠️  Twitter API authentication failed for query '{query}' with method {auth_method[:20]}...")
                            continue  # Try next auth method
                        elif response and response.status_code == 429:
                            print(f"    ⚠️  Twitter API rate limit exceeded for query '{query}' - continuing with available data")
                            social_data[query] = {'data': []}
                            break  # Rate limited, stop trying
                        else:
                            print(f"    ⚠️  Twitter API failed for query '{query}': {response.status_code if response else 'No response'}")
                            continue  # Try next auth method
                            
                    except Exception as e:
                        print(f"    ⚠️  Twitter API error for query '{query}': {e}")
                        continue  # Try next auth method
                
                # If we got some data, break out of auth method loop
                if social_data:
                    break
            
            # If no data found, return empty structure
            if not social_data:
                print(f"    ❌ Twitter API failed for all queries and auth methods")
                social_data = {'data': []}
    ...
    def fetch_telegram_social_data(self, token_symbol, token_name):
        ...
        try:
            ...
            if response and response.status_code == 200:
                data = response.json()
                print(f"    ✅ Telegram data found")
                return data
            else:
                print(f"    ❌ Telegram API failed: {response.status_code if response else 'No response'}")
                return {}
    ...
    def fetch_discord_social_data(self, token_symbol, token_name):
        ...
        try:
            ...
            # Try to fetch real Discord data
            ...
            if response and response.status_code == 200:
                data = response.json()
                print(f"    ✅ Discord data found")
                return data
            elif response and response.status_code == 401:
                print(f"    ❌ Discord API authentication failed: Invalid bot token")
                return {}
            else:
                print(f"    ❌ Discord API failed: {response.status_code if response else 'No response'}")
                return {}
        ...
        try:
            ...
            if response and response.status_code == 200:
                ...
                if access_token:
                    ...
                    search_url = f"https://oauth.reddit.com/search?q={token_symbol}&limit=10"
                    headers = {"Authorization": f"Bearer {access_token}", "User-Agent": "DeFiRiskAssessor/3.0"}
                    search_response = robust_request('GET', search_url, headers=headers, timeout=15)
                    ...
                    if search_response and search_response.status_code == 200:
                        ...
                        print(f"    ❌ Reddit search failed: {search_response.status_code if search_response else 'No response'}")
                        return {}
                else:
                    print(f"    ❌ Reddit authentication failed")
                    return {}
            else:
                print(f"    ❌ Reddit API failed: {response.status_code if response else 'No response'}")
                return {}
    ...
    def fetch_arkham_intelligence_data(self, token_address, token_symbol):
        ...
        try:
            ...
            url = f"https://api.arkhamintelligence.com/intelligence/entity"
            headers = {
            ...
            response = self.robust_request('GET', url, headers=headers, params=params, timeout=15)
            ...
            if response and response.status_code == 200:
                ...
                print(f"    ❌ Arkham API failed: {response.status_code if response else 'No response'}")
                return {'placeholder': True}
    ...
    def fetch_coindesk_social_data(self, token_symbol, token_name):
        ...
        try:
            ...
            # CoinDesk RSS feed for crypto news
            url = "https://www.coindesk.com/arc/outboundfeeds/rss/"
            headers = {"User-Agent": "DeFiRiskAssessor/3.0"}
            
            response = self.robust_request('GET', url, headers=headers, timeout=15)
            
            if response and response.status_code == 200:
                # Parse RSS feed for mentions of the token
                ...
                try:
                    ...
                    for item in root.findall('.//item'):
                        ...
                        if title is not None and description is not None:
                            ...
                            desc_text = description.text.lower() if description.text else ""
                            
                            # Check for token mentions
                            if token_symbol.lower() in title_text or token_symbol.lower() in desc_text:
                                link_elem = item.find('link')
                                link_text = link_elem.text if link_elem is not None and link_elem.text else ""
                                pub_elem = item.find('pubDate')
                                pub_text = pub_elem.text if pub_elem is not None and pub_elem.text else ""
                                mentions.append({
                                    'title': title.text,
                    ...
                    print(f"    ❌ CoinDesk RSS parsing failed")
                    return {'mention_count': 0, 'sentiment': 'neutral'}
            else:
                print(f"    ❌ CoinDesk API failed: {response.status_code if response else 'No response'}")
                return {'mention_count': 0, 'sentiment': 'neutral'}
    ...
    def fetch_theblock_social_data(self, token_symbol, token_name):
        ...
        try:
            ...
            # The Block API endpoint for crypto news
            url = "https://www.theblock.co/api/content"
            headers = {"User-Agent": "DeFiRiskAssessor/3.0"}
            params = {
                'query': token_symbol,
                'limit': 10,
            ...
            if response and response.status_code == 200:
                ...
                if articles:
                    ...
                    print(f"    ⚠️  No The Block articles found")
                    return {'article_count': 0, 'sentiment': 'neutral'}
            else:
                print(f"    ❌ The Block API failed: {response.status_code if response else 'No response'}")
                return {'article_count': 0, 'sentiment': 'neutral'}
    ...
    def fetch_decrypt_social_data(self, token_symbol, token_name):
        ...
        try:
            ...
            # Decrypt RSS feed for crypto news (no API available)
            ...
            if response and response.status_code == 200:
                ...
                try:
                    ...
                    print(f"    ❌ Decrypt RSS parsing failed")
                    return {'mention_count': 0, 'sentiment': 'neutral'}
            else:
                print(f"    ❌ Decrypt RSS failed: {response.status_code if response else 'No response'}")
                return {'mention_count': 0, 'sentiment': 'neutral'}
    ...
    def fetch_defillama_market_data(self, token_symbol, token_name):
        ...
        try:
            ...
            protocol_url = f"https://api.llama.fi/protocol/{token_symbol.lower()}"
            response = self.robust_request('GET', protocol_url, headers=headers, timeout=15)
            ...
            tvl_response = self.robust_request('GET', tvl_url, headers=headers, timeout=15)
            
            tvl_data = {}
            if tvl_response and tvl_response.status_code == 200:
                protocols = tvl_response.json()
            ...
            yield_response = self.robust_request('GET', yield_url, headers=headers, timeout=15)
            
            yield_data = []
            if yield_response and yield_response.status_code == 200:
                pools = yield_response.json()
                # Filter pools that might be related to this token
                for pool in pools:
                    if token_symbol.lower() in pool.get('symbol', '').lower():
            ...
            return {
                ...
                'has_protocol': bool(protocol_data),
                'tvl': tvl_data.get('tvl', 0) if tvl_data else 0,
                'yield_pools_count': len(yield_data)
            }
        ...
        try:
            ...
            for protocol, subgraph_id in subgraphs.items():
                try:
                    ...
                    # Comprehensive query for each protocol
                    query = {
                        "query": f"""
                        {{
                          ...
                          pools(where: {{token0: "{token_address.lower()}"}}) {{
                            id
                            liquidity
                            volumeUSD
                            token0Price
                          ...
                          pools(where: {{token1: "{token_address.lower()}"}}) {{
                            id
                            liquidity
                            volumeUSD
                            token0Price
                    ...
                    response = self.robust_request('POST', url, json=query, headers=headers, timeout=15)
                    
                    if response and response.status_code == 200:
                        data = response.json()
                        if data.get('data'):
                            all_data[protocol] = data['data']
                            print(f"    ✅ {protocol} data found")
                        else:
                            print(f"    ⚠️  No {protocol} data for this token")
                    else:
                        print(f"    ❌ {protocol} API failed: {response.status_code if response else 'No response'}")
            ...
            if all_data:
                print(f"    ✅ The Graph data collected from {len(all_data)} protocols")
                return {
                    'protocols': all_data,
                    'total_protocols': len(all_data),
                    'has_liquidity': any('pools' in data for data in all_data.values()),
                    'total_volume': sum(
                        float(data.get('token', {}).get('tradeVolumeUSD', 0) or 0) 
                        for data in all_data.values()
                    ),
                    'total_liquidity': sum(
                        float(data.get('token', {}).get('totalLiquidity', 0) or 0) 
                        for data in all_data.values()
                    )
                }
            else:
                print(f"    ⚠️  No The Graph data found for any protocol")
                return {'placeholder': True}
    ...
    def _init_sentiment_engine(self):
        """Initialize sentiment dictionaries used for social/sentiment analysis."""
    ...
    def fetch_bitcointalk_sentiment(self, token_symbol, token_name):
        ...
        try:
            ...
            response = self.robust_request('GET', search_url, params=params, headers=headers, timeout=15)
    ...
    def fetch_cointelegraph_news(self, token_symbol, token_name):
        ...
        try:
            if not COINTELEGRAPH_API_KEY:
                print(f"  ⚠️  Cointelegraph API key missing, using fallback")
    ...
    def fetch_cointelegraph_rss(self, token_symbol, token_name):
        ...
        try:
            ...
            response = self.robust_request('GET', rss_url, headers=headers, timeout=15)
            ...
            if response and response.status_code == 200:
                ...
                for item in root.findall('.//item')[:10]:  # Get first 10 items
                    ...
                    if title is not None and description is not None:
                        ...
                        if (token_symbol.lower() in title_text.lower() or 
                            token_symbol.lower() in desc_text.lower() or
                            token_name.lower() in title_text.lower() or 
                            token_name.lower() in desc_text.lower()):
                            articles.append({
                                'title': title_text,
...
def process_token_batch(input_file=None, output_file=None, json_output_file=None):
    ...
    try:
        if not hasattr(analyzer, 'stats') or not isinstance(getattr(analyzer, 'stats', None), dict):
            analyzer.stats = {
                'successful_assessments': 0,
                'failed_assessments': 0,
                'cache_hits': 0,
        ...
        if not hasattr(analyzer, 'w3') or analyzer.w3 is None:
            try:
                if INFURA_API_KEY:
                    analyzer.w3 = Web3(Web3.HTTPProvider(f"https://mainnet.infura.io/v3/{INFURA_API_KEY}"))
                else:
                    analyzer.w3 = None
            except Exception:
                analyzer.w3 = None
        ...
        if not hasattr(analyzer, 'WEIGHTS') or analyzer.WEIGHTS is None:
            analyzer.WEIGHTS = {
                "industry_impact": 0.08,
                "tech_innovation": 0.08,
                "whitepaper_quality": 0.06,
        ...
        if not hasattr(analyzer, 'CHAIN_CONFIG') or analyzer.CHAIN_CONFIG is None:
            analyzer.CHAIN_CONFIG = {
                "eth": {
                    "rpc": f"https://mainnet.infura.io/v3/{os.getenv('INFURA_API_KEY')}",
                    "scan_url": "https://api.etherscan.io/v2/api",
                    "scan_key": os.getenv("ETHERSCAN_API_KEY"),
    ...
    def process_one(token, token_index, total_tokens):
        ...
        try:
            ...
            summary = {
                ...
                "total_score_minus_social": risk_scores.get('total_score_minus_social', overall_score),
                ...
                "is_stablecoin": analyzer._detect_stablecoin(result.get('token', ''), result.get('symbol', ''), result.get('market_data', {})),
                ...
                "key_metrics": analyzer._extract_key_metrics(result),
    ...
    try:
        ...
        with pd.ExcelWriter(excel_file, engine='openpyxl') as writer:
            ...
            for row in range(2, len(excel_data) + 2):  # Start from row 2 (skip header)
                # Color EU Regulatory Issues column (orange) - now column 17 (Q)
                eu_issues_cell = worksheet.cell(row=row, column=17)  # Column Q
                if eu_issues_cell.value == 'Yes':
                    eu_issues_cell.fill = orange_fill
                
                # Color other Non-Compliant statuses (red) - adjusted for new timestamp column
                for col in [13, 14, 15, 16, 18, 19, 20, 21]:  # Other red flag columns (adjusted)
                    cell = worksheet.cell(row=row, column=col)
                    if cell.value == 'Yes':
                        cell.fill = red_fill
                
                # Color Risk Category column - now column 10 (J)
                risk_cell = worksheet.cell(row=row, column=10)  # Column J
    ...
    api_usage = {
        ...
        'market_data': ['CoinGecko', 'CoinMarketCap', 'CoinPaprika', 'DeFiLlama', '1inch', 'Etherscan', 'Ethplorer']
    }
...
def fetch_1inch_token_metadata(token_address, chain_id=1):
    ...
    try:
        # Try the correct 1inch API endpoint
        url = f"https://api.1inch.dev/token/v1.0/{chain_id}/metadata"
        headers = {"Authorization": f"Bearer {INCH_API_KEY}"}
        params = {"address": token_address}
        response = robust_request('GET', url, headers=headers, params=params, timeout=15)
        ...
        if response and response.status_code == 200:
            ...
            try:
                alt_url = f"https://api.1inch.dev/token/v1.0/{chain_id}/metadata"
                response = robust_request('GET', alt_url, headers=headers, params=params, timeout=15)
                if response and response.status_code == 200:
                    data = response.json()
                    print(f"      ✅ 1inch token metadata found (alternative)")
                    return data
                else:
                    print(f"      ❌ 1inch Token Metadata API failed: {response.status_code if response else 'No response'}")
                    return None
            except Exception:
                print(f"      ❌ 1inch API connectivity issue")
                return None
    except Exception as e:
...
def fetch_1inch_comprehensive_data(token_address, chain_id=1):
    ...
    try:
        ...
        # 1. Spot Price
        price_url = f"https://api.1inch.dev/price/v1.1/{chain_id}"
        price_params = {"tokenAddress": token_address}
        response = robust_request('GET', price_url, headers=headers, params=price_params, timeout=15)
        if response and response.status_code == 200:
            all_data['price'] = response.json()
            print(f"      ✅ 1inch spot price found")
        
        # 2. Token Metadata
        metadata_url = f"https://api.1inch.dev/token/v1.0/{chain_id}/metadata"
        metadata_params = {"address": token_address}
        response = robust_request('GET', metadata_url, headers=headers, params=metadata_params, timeout=15)
        if response and response.status_code == 200:
            all_data['metadata'] = response.json()
            print(f"      ✅ 1inch token metadata found")
        
        # 3. Available Tokens (for liquidity analysis)
        tokens_url = f"https://api.1inch.dev/swap/v6.0/{chain_id}/tokens"
        response = robust_request('GET', tokens_url, headers=headers, timeout=15)
        if response and response.status_code == 200:
            tokens_data = response.json()
            # Check if our token is in the list
            if 'tokens' in tokens_data:
                for token in tokens_data['tokens'].values():
        ...
        # 4. Liquidity Sources
        liquidity_url = f"https://api.1inch.dev/swap/v6.0/{chain_id}/liquidity-sources"
        response = robust_request('GET', liquidity_url, headers=headers, timeout=15)
        if response and response.status_code == 200:
            all_data['liquidity_sources'] = response.json()
            print(f"      ✅ 1inch liquidity sources found")
        
        # 5. Protocol Data
        protocols_url = f"https://api.1inch.dev/swap/v6.0/{chain_id}/protocols"
        response = robust_request('GET', protocols_url, headers=headers, timeout=15)
        if response and response.status_code == 200:
...
def fetch_scorechain_aml(token_address, chain):
    """Fetch AML data from Scorechain - Using placeholder until API key is implemented"""
    # Placeholder implementation - return default score of 5/10
...
def fetch_trmlabs_aml(token_address, chain):
    """Fetch AML data from TRM Labs - Using placeholder until API key is implemented"""
    # Placeholder implementation - return default score of 5/10
...
def fetch_opensanctions_compliance(token_address, chain):
    """Fetch compliance data from OpenSanctions - Using placeholder until API key is implemented"""
    # Placeholder implementation - return default score of 5/10
...
def fetch_lukka_compliance(token_address, chain):
    """Fetch compliance data from Lukka - Using placeholder until API key is implemented"""
    # Placeholder implementation - return default score of 5/10
...
def fetch_alchemy_comprehensive_data(token_address, chain):
    ...
    try:
        ...
        response = robust_request('POST', url, headers=headers, json=metadata_data, timeout=15)
        if response and response.status_code == 200:
            all_data['metadata'] = response.json()
        ...
        response = robust_request('POST', url, headers=headers, json=balances_data, timeout=15)
        if response and response.status_code == 200:
        ...
        response = robust_request('POST', url, headers=headers, json=tx_data, timeout=15)
        if response and response.status_code == 200:
        ...
        response = robust_request('POST', url, headers=headers, json=supply_data, timeout=15)
        if response and response.status_code == 200:
...
def fetch_defisafety_compliance(token_address, chain):
    """Fetch compliance data from DeFiSafety - Using placeholder until API key is implemented"""
    # Placeholder implementation - return default score of 5/10
...
def fetch_certik_security(token_address, chain):
    """Fetch security data from CertiK - Using placeholder until API key is implemented"""
    # Placeholder implementation - return default score of 5/10
    ...
    try:
        ...
        if response and response.status_code == 200:
            ...
            try:
                ...
                response = robust_request('GET', fallback_url, headers=headers, timeout=15)
                if response and response.status_code == 200:
                    data = response.json()
...
def validate_api_keys_comprehensive():
    ...
    if BITQUERY_API_KEY:
        ...
        print(f"  • BitQuery: ✅ Configured")
    else:
        api_status['onchain_data']['bitquery'] = "❌ Not configured"
        print(f"  • BitQuery: ❌ Not configured")
...
def generate_social_score_report():
    """Generate a comprehensive social score analysis report"""
    try:
        # Load the latest risk assessment results
        csv_file = os.path.join(DATA_DIR, 'risk_report.csv')
        if not os.path.exists(csv_file):
            print("❌ No risk report found for social analysis")
            return
        ...
        # Analyze each token's social data
        for index, row in df.iterrows():
            token_name = row.get('token_name', 'Unknown')
            symbol = row.get('symbol', 'Unknown')
            
            # Parse social data from the row
            social_data_str = row.get('social_data', '{}')
            try:
                if isinstance(social_data_str, str):
                    social_data = json.loads(social_data_str)
                else:
                    social_data = social_data_str
            except:
                social_data = {}
            
            # Normalize social data structure
            if not isinstance(social_data, dict):
                social_data = {}
            
            # Analyze each social source
            for source, source_data in social_analysis['social_sources'].items():
                if source in social_data and social_data[source]:
                    source_data['found'] += 1
                    
                    # Extract words and queries used
                    if isinstance(social_data[source], dict):
                        # Look for queries, keywords, or text content
                        for key, value in social_data[source].items():
                            if isinstance(value, str):
                                words = value.lower().split()
                                for word in words:
                                    if len(word) > 2:  # Skip short words
                                        source_data['words'][word] = source_data['words'].get(word, 0) + 1
                                        social_analysis['word_frequency'][word] = social_analysis['word_frequency'].get(word, 0) + 1
                else:
                    source_data['not_found'] += 1
        
        # Generate the report
        timestamp = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
        report_file = os.path.join(DATA_DIR, f'social_score_report_{timestamp}.txt')
        ...
        with open(report_file, 'w') as f:
            ...
            # Most Frequent Words
            f.write("MOST FREQUENT WORDS FOUND\n")
            f.write("-" * 40 + "\n")
            sorted_words = sorted(social_analysis['word_frequency'].items(), key=lambda x: x[1], reverse=True)
            for word, count in sorted_words[:20]:
                f.write(f"{word:20} | Count: {count:3d}\n")
            f.write("\n")
            ...
            for source, data in social_analysis['social_sources'].items():
                if data['words']:
                    f.write(f"\n{source.upper()}:\n")
                    sorted_source_words = sorted(data['words'].items(), key=lambda x: x[1], reverse=True)
            ...
            f.write("\nWord Recommendations:\n")
            if sorted_words:
                f.write(f"  🔍 Most effective words: {', '.join([word for word, _ in sorted_words[:5]])}\n")
                f.write(f"  ⚠️  Consider replacing: {', '.join([word for word, _ in sorted_words[-5:] if word not in ['the', 'and', 'for', 'with', 'this']])}\n")
