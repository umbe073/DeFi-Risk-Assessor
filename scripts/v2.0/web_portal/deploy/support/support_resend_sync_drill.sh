...
DRILL_RUNS="${SUPPORT_SYNC_DRILL_RUNS:-3}"
DRILL_SLEEP_SECONDS="${SUPPORT_SYNC_DRILL_SLEEP_SECONDS:-5}"
...
if [[ ! -f "${ENV_FILE}" ]]; then
  echo "[support-resend-sync-drill] missing env file: ${ENV_FILE}" >&2
  exit 1
fi
...
if ! to_bool "${routing_active_raw}"; then
  echo "[support-resend-sync-drill] skipped (SUPPORT_INBOUND_ROUTING_ACTIVE is false)"
  exit 0
fi

if [[ -z "${webhook_secret}" ]]; then
  echo "[support-resend-sync-drill] missing SUPPORT_INBOUND_WEBHOOK_SECRET" >&2
  exit 1
fi
if [[ -z "${base_url}" ]]; then
...
if [[ -z "${DRILL_RUNS}" || ! "${DRILL_RUNS}" =~ ^[0-9]+$ ]]; then
  DRILL_RUNS="3"
fi
if (( DRILL_RUNS < 1 )); then
  DRILL_RUNS=1
fi
if [[ -z "${DRILL_SLEEP_SECONDS}" || ! "${DRILL_SLEEP_SECONDS}" =~ ^[0-9]+$ ]]; then
  DRILL_SLEEP_SECONDS="5"
fi

sync_url="${base_url%/}/api/v1/support/inbound-email/resend/sync"
health_url="${base_url%/}/healthz"
echo "[support-resend-sync-drill] starting runs=${DRILL_RUNS} sleep=${DRILL_SLEEP_SECONDS}s limit=${limit_raw} url=${sync_url}"
...
if (( ready != 1 )); then
  echo "[support-resend-sync-drill] app not ready url=${health_url}" >&2
  exit 7
fi

sustained_detected=0
for run_idx in $(seq 1 "${DRILL_RUNS}"); do
  body_file="$(mktemp /tmp/support-resend-sync-drill.XXXXXX.json)"
  ...
  if [[ "${http_code}" != "200" ]]; then
    echo "[support-resend-sync-drill] run=${run_idx} failed http=${http_code}" >&2
...
print(
    f"run_id={int(payload.get('run_id', 0) or 0)}"
    ...
    f" breach={1 if bool(policy.get('current_breach', False)) else 0}"
    f" sustained={1 if bool(policy.get('sustained_breach', False)) else 0}"
)
PY
  )"
  echo "[support-resend-sync-drill] run=${run_idx} ${summary}"
  if [[ "${summary}" == *"sustained=1"* ]]; then
    sustained_detected=1
  fi
  rm -f "${body_file}" || true
  trap - EXIT
  if (( run_idx < DRILL_RUNS )); then
    sleep "${DRILL_SLEEP_SECONDS}"
  fi
done

if (( sustained_detected == 1 )); then
  echo "[support-resend-sync-drill] policy=ALERT sustained_unmapped_detected"
  exit 2
fi
echo "[support-resend-sync-drill] policy=PASS no_sustained_unmapped_detected"
