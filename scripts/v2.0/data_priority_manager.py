...
Implements the correct data priority system: webhook cache -> fallback -> real-time API -> risk report
"""
...
class DataPriorityManager:
    """Manages data fetching with proper priority system"""
    ...
    def get_token_data(self, token_address: str, symbol: str = None) -> Dict[str, Any]:
        """
        Get token data following the priority system:
        ...
        """
        print(f"🔍 Getting data for {symbol or 'Unknown'} ({token_address})")
        ...
        # Step 1: Check webhook cache first
        ...
        if missing_fields:
            ...
            real_time_data = self._fetch_real_time_data(token_address, missing_fields)
            if real_time_data:
    ...
    def _get_webhook_cache_data(self, token_address: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Check direct address keys
            if token_address.lower() in cache_data:
                token_data = cache_data[token_address.lower()]
                return self._extract_webhook_cache_values(token_data)
    ...
    def _extract_webhook_cache_values(self, token_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for source, data in market_data.items():
            if data.get('source') == 'fallback':
                ...
                return {}  # Return empty to force fallback to next priority
    ...
    def _fetch_real_time_data(self, token_address: str, missing_fields: List[str]) -> Optional[Dict[str, Any]]:
...
def test_data_priority_manager():
    """Test the data priority manager"""
    manager = DataPriorityManager()
    ...
    for address, symbol in test_tokens:
        ...
        data = manager.get_token_data(address, symbol)
        print(f"  Result: {data}")
