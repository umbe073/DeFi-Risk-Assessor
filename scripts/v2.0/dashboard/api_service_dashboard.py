...
API_RUNTIME_DIR = os.path.join(DATA_DIR, 'api_runtime')
SERVICE_TOGGLES_FILE = os.path.join(API_RUNTIME_DIR, 'service_toggles.json')
API_SERVICE_REGISTRY_FILE = os.path.join(API_RUNTIME_DIR, 'api_services_registry.json')

# Canonical homepage URLs used by About window and docs links.
SERVICE_HOMEPAGE_URLS: Dict[str, str] = {
...
class APIServiceDashboard:
    def __init__(self):
        ...
        self.setup_window()
        self.services: Dict[str, Dict[str, Any]] = self.initialize_services()
        self.service_widgets: Dict[str, Dict[str, Any]] = {}
        self.load_service_toggles()
        self.export_service_registry()
    ...
    def _service_enabled_default(self, service):
        """Resolve default enabled state for a service."""
        if not isinstance(service, dict):
            return True
        if 'enabled_default' in service:
            return bool(service.get('enabled_default'))
        return True

    def load_service_toggles(self):
        """Load persisted per-service enable/disable state."""
        raw = {}
        try:
            if os.path.exists(SERVICE_TOGGLES_FILE):
                with open(SERVICE_TOGGLES_FILE, 'r', encoding='utf-8') as f:
                    payload = json.load(f)
                if isinstance(payload, dict):
                    raw = payload.get('services', payload)
                    if not isinstance(raw, dict):
                        raw = {}
        except Exception as e:
            print(f"Warning: could not load service toggles: {e}")
            raw = {}

        for service_id, service in self.services.items():
            default_enabled = self._service_enabled_default(service)
            enabled = bool(raw.get(service_id, default_enabled))
            service['enabled'] = enabled

    def save_service_toggles(self):
        """Persist per-service enable/disable state."""
        try:
            os.makedirs(API_RUNTIME_DIR, exist_ok=True)
            payload = {
                'updated_at': datetime.now().isoformat(),
                'services': {
                    service_id: bool(service.get('enabled', True))
                    for service_id, service in self.services.items()
                }
            }
            with open(SERVICE_TOGGLES_FILE, 'w', encoding='utf-8') as f:
                json.dump(payload, f, indent=2)
        except Exception as e:
            print(f"Warning: could not save service toggles: {e}")

    def set_service_enabled(self, service_id, enabled):
        """Set one service enabled/disabled and persist it."""
        service = self.services.get(service_id)
        if not service:
            return
        service['enabled'] = bool(enabled)
        self.save_service_toggles()
        self.export_service_registry()
        self.update_service_status()

    def set_all_services_enabled(self, enabled):
        """Bulk enable/disable all services."""
        target_state = bool(enabled)
        for service in self.services.values():
            service['enabled'] = target_state
        for widget in self.service_widgets.values():
            toggle_var = widget.get('enabled_var')
            if toggle_var is not None:
                toggle_var.set(target_state)
        self.save_service_toggles()
        self.export_service_registry()
        self.update_service_status()

    @staticmethod
    def _derive_service_reference_url(service_id: str, service: Dict[str, Any]) -> str:
        """Best-effort homepage URL for About window dynamic listing."""
        sid = str(service_id or "").strip().lower()
        if sid in SERVICE_HOMEPAGE_URLS:
            return SERVICE_HOMEPAGE_URLS[sid]

        homepage = str(service.get('homepage_url') or '').strip()
        if homepage.startswith('http://') or homepage.startswith('https://'):
            return homepage

        endpoint = str(service.get('test_endpoint') or '').strip()
        if not (endpoint.startswith('http://') or endpoint.startswith('https://')):
            return ""

        # Fallback to host root if we do not know this service explicitly.
        parsed = urlparse(endpoint)
        if not parsed.scheme or not parsed.netloc:
            return ""
        return f"{parsed.scheme}://{parsed.netloc}/"

    def export_service_registry(self):
        """Persist API service catalog for cross-window dynamic UI (About window)."""
        try:
            os.makedirs(API_RUNTIME_DIR, exist_ok=True)
            services_payload = []
            for service_id, service in self.services.items():
                services_payload.append({
                    ...
                    'reference_url': self._derive_service_reference_url(service_id, service),
                })
            services_payload.sort(key=lambda s: (str(s.get('category', '')), str(s.get('name', ''))))
            payload = {
                'updated_at': datetime.now().isoformat(),
                'service_count': len(services_payload),
                'services': services_payload,
            }
            with open(API_SERVICE_REGISTRY_FILE, 'w', encoding='utf-8') as f:
                json.dump(payload, f, indent=2)
        except Exception as e:
            print(f"Warning: could not export API service registry: {e}")
        
    def initialize_services(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            'coincap': {
                ...
                'last_call': 0,
                'calls_count': 0,
                'test_endpoint': 'https://rest.coincap.io/v3/assets/bitcoin',
                'description': 'Market data endpoint with optional API-key auth and monthly credit limits'
            },
            'dexscreener': {
                ...
                'last_call': 0,
                'calls_count': 0,
                'test_endpoint': 'https://api.dexscreener.com/latest/dex/tokens/So11111111111111111111111111111111111111112',
                'description': 'DEX-native market data fallback (price, volume, liquidity, market cap)'
            },
            'birdeye': {
                'name': 'BirdEye API',
                ...
                'last_call': 0,
                'calls_count': 0,
                'test_endpoint': 'https://public-api.birdeye.so/defi/token_overview',
                'description': 'Solana + multichain market intelligence (token overview, liquidity, holders)'
            },
            'coinpaprika': {
            ...
            'solscan': {
                ...
                'alt_env_keys': ['SOLSCAN_PRO_API_KEY'],
                'category': '📈 Blockchain Analytics',
                'rate_limit': 60,
                'rate_period': 60,
                'last_call': 0,
                'calls_count': 0,
                'test_endpoint': 'https://pro-api.solscan.io/v2.0/token/meta',
                'description': 'Solana token metadata and holder analytics'
            },
            'solanatracker': {
                ...
                'last_call': 0,
                'calls_count': 0,
                'enabled_default': str(os.getenv('SOLANATRACKER_ENABLED', '0')).strip().lower() in {'1', 'true', 'yes', 'on'},
                'test_endpoint': 'https://data.solanatracker.io/tokens/So11111111111111111111111111111111111111112/holders',
                'description': 'Solana holder analytics and token-level distribution insights'
            },
            ...
            'lifi': {
                ...
                'last_call': 0,
                'calls_count': 0,
                'test_endpoint': 'https://li.quest/v1/token?chain=1&token=0xdAC17F958D2ee523a2206206994597C13D831ec7',
                'description': 'Cross-chain liquidity routing and token intelligence'
            },
            ...
            'breadcrumbs': {
                ...
                'last_call': 0,
                'calls_count': 0,
                'test_endpoint': 'https://api.breadcrumbs.one/sanctions/address',
                'description': 'Sanctions & illicit-activity screening (sanctions endpoint)'
            },
            'certik': {
                'name': 'CertiK API',
            ...
            'honeypot': {
                ...
                'last_call': 0,
                'calls_count': 0,
                'test_endpoint': 'https://api.honeypot.is/v2/IsHoneypot?address=0xdAC17F958D2ee523a2206206994597C13D831ec7',
                'description': 'Smart-contract honeypot and tax-simulation risk checks (free, no API key)'
            },
            'chainalysis_oracle': {
                ...
                'last_call': 0,
                'calls_count': 0,
                'test_endpoint': 'https://cloudflare-eth.com',
                'description': 'On-chain sanctions screening via oracle contract isSanctioned(address) (free, no API key)'
            },
            'ofac_sls': {
                'name': 'OFAC SLS (Datasets)',
                'env_key': None,
                'category': '🛡️ Security & Compliance',
                'rate_limit': 30,
                'rate_period': 60,
                'last_call': 0,
                'calls_count': 0,
                'test_endpoint': 'https://sanctionslistservice.ofac.treas.gov/api/publicationpreview/exports/add.csv',
                'description': 'Official sanctions datasets (download + local cache strategy)'
            },
            'scamsniffer': {
                ...
                'last_call': 0,
                'calls_count': 0,
                'test_endpoint': 'https://raw.githubusercontent.com/scamsniffer/scam-database/main/blacklist/address.json',
                'description': 'Open blacklist of scam addresses/domains (free GitHub dataset)'
            },
            ...
            'trmlabs': {
                ...
                'last_call': 0,
                'calls_count': 0,
                'test_endpoint': 'https://api.sanctions.trmlabs.com/v1/screen',
                'description': 'Sanctions screening (public endpoint: 1 req/sec, 100 req/day; higher limits with API key)'
            },
            'chainabuse': {
                'name': 'Chainabuse API',
            ...
            'dune': {
                ...
                'last_call': 0,
                'calls_count': 0,
                'test_endpoint': 'https://api.sim.dune.com/v1/evm/token-info/{address}?chain_ids={chain_id}',
                'description': 'SIM Dune blockchain data API (token intelligence, balances, activity)'
            },
            'bitcointalk': {
    ...
    def create_widgets(self):
        ...
        enable_all_btn = ttk.Button(
            control_frame,
            text="✅ Enable All",
            command=lambda: self.set_all_services_enabled(True)
        )
        enable_all_btn.grid(row=0, column=3, padx=(0, 10))

        disable_all_btn = ttk.Button(
            control_frame,
            text="⛔ Disable All",
            command=lambda: self.set_all_services_enabled(False)
        )
        disable_all_btn.grid(row=0, column=4, padx=(0, 10))
    ...
    def create_service_widgets(self):
        ...
        for category, services in categories.items():
            ...
            for service_id, service in services:
                ...
                enabled_var = tk.BooleanVar(value=bool(service.get('enabled', True)))
                enable_cb = ttk.Checkbutton(
                    service_frame,
                    text="Enabled",
                    variable=enabled_var,
                    command=lambda sid=service_id, v=enabled_var: self.set_service_enabled(sid, v.get())
                )
                enable_cb.grid(row=0, column=3, padx=(0, 10))
                
                # Manual trigger button
                trigger_btn = ttk.Button(service_frame, text="🔄 Fetch Data", 
                                       command=lambda sid=service_id: self.trigger_service(sid))
                trigger_btn.grid(row=0, column=4)
    ...
    def get_service_status(self, service_id):
        ...
        if not bool(service.get('enabled', True)):
            return 'disabled', 'Disabled (manual)'
        ...
        if service['env_key']:
            ...
            if not api_key:
                for alt_env in service.get('alt_env_keys', []) or []:
                    api_key = os.getenv(alt_env)
                    if api_key:
                        break
    ...
    def update_service_status(self):
        ...
        for service_id, widgets in self.service_widgets.items():
            ...
            if status == 'available':
                ...
                color = '#7f8c8d'  # Gray
                widgets['countdown_label'].config(text=message, style='Status.TLabel')
                widgets['trigger_btn'].config(state='disabled')
            else:  # unavailable
                color = '#e74c3c'  # Red
    ...
    def _fetch_service_data(self, service_id):  # pyright: ignore[reportGeneralTypeIssues]
        ...
        try:
            ...
            if service['test_endpoint']:
                ...
                trmlabs_override = (os.getenv('TRMLABS_SANCTIONS_ENDPOINT') or '').strip() if service_id == 'trmlabs' else ''
                ...
                if service_id == 'infura' and api_key:
                ...
                elif api_key or service_id in {'chainalysis_oracle', 'trmlabs'}:
                    ...
                    if service_id == 'moralis':
                        ...
                        for candidate_url, candidate_headers, candidate_params in candidate_requests:
                            ...
                            # Keep probing alternate hosts/auth styles for expected auth/shape errors.
                        ...
                        if trmlabs_override:
                            if trmlabs_override.startswith('http'):
                                normalized_override = trmlabs_override.rstrip('/')
                            else:
                                normalized_override = f"https://{trmlabs_override.strip('/')}"
                            if not (
                                normalized_override.endswith('/v1/screen')
                                or normalized_override.endswith('/public/v1/sanctions/screening')
                            ):
                                normalized_override = f"{normalized_override}/v1/screen"
                        else:
                            normalized_override = ''
                        candidate_urls = [
                            normalized_override or 'https://api.sanctions.trmlabs.com/v1/screen',
                            'https://api.sanctions.trmlabs.com/v1/screen',
                        ...
                        for candidate_url in candidate_urls:
                            for payload in payload_candidates:
                                ...
                                if candidate_response.status_code in [400, 422]:
                                    continue
                        ...
                        if response is None:
                            response = best_response
                    elif service_id == 'chainalysis_oracle':
                        headers['Content-Type'] = 'application/json'
                        test_address = '0xdAC17F958D2ee523a2206206994597C13D831ec7'
                        selector = 'df592f7d'
                        encoded_address = test_address.lower().replace('0x', '').rjust(64, '0')
                        payload = {
                            'jsonrpc': '2.0',
                            'id': 1,
                            'method': 'eth_call',
                            'params': [
                                {
                                    'to': '0x40C579239B58A3B4cC4cd56dD9f30220d8C15438',
                        ...
                        rpc_candidates = []
                        override_rpc = (os.getenv('CHAINALYSIS_ORACLE_RPC_URL') or '').strip()
                        if override_rpc:
                            rpc_candidates.append(override_rpc)
                        env_rpc = (os.getenv('ETH_RPC_URL') or '').strip()
                        if env_rpc:
                            rpc_candidates.append(env_rpc)
                        infura_key = (os.getenv('INFURA_API_KEY') or '').strip()
                        if infura_key:
                        ...
                        rpc_candidates = list(dict.fromkeys([u.rstrip('/') for u in rpc_candidates if u]))
                        response = None
                        best_response = None
                        ...
                        normalized_key = api_key.strip()
                        headers['X-Sim-Api-Key'] = normalized_key
                        headers['Accept'] = 'application/json'
                        ...
                        response = requests.get(
                            dune_url,
                            headers=headers,
                            params={'chain_ids': dune_chain_id, 'limit': 1},
                            timeout=15
                        )
                    ...
                    if service_id not in ['santiment', 'thegraph', 'zapper', 'dune', 'breadcrumbs', 'oklink', 'trmlabs', 'chainabuse', 'chainalysis_oracle', 'birdeye', 'solscan', 'solanatracker']:  # These services handle their own requests
                        response = requests.get(url, headers=headers, timeout=10)
                
                else:
                    # No API key or service doesn't need one
                    if service_id not in ['infura', 'alchemy', 'zapper', 'oneinch', 'lifi', 'twitter', 'reddit', 'breadcrumbs', 'thegraph', 'dune', 'oklink', 'trmlabs', 'chainabuse', 'birdeye', 'solscan', 'solanatracker']:
                        response = requests.get(url, headers=headers, timeout=10)
                    else:
                ...
                if response is not None:
                    if response.status_code in [200, 201]:
                        if service_id == 'oklink':
                            ...
                            try:
                                ...
                                if isinstance(payload, dict) and isinstance(payload.get('result'), str):
                                    success = True
                                    message = f"✅ Success: {service['name']} oracle call responded correctly"
                                elif isinstance(payload, dict) and payload.get('error'):
                                    success = True
                                    message = f"⚠️ {service['name']}: RPC reachable (oracle call returned error payload)"
                                else:
                                    success = True
                                    message = f"⚠️ {service['name']}: RPC reachable (unexpected payload shape)"
                            except Exception:
                                success = True
                                message = f"⚠️ {service['name']}: RPC reachable but JSON parsing failed"
                        elif service_id == 'ofac_sls':
                            body_len = len(response.text or '')
                            if body_len > 100:
                                success = True
                                message = f"✅ Success: {service['name']} dataset download endpoint responded"
                            else:
                                success = True
                                message = f"⚠️ {service['name']}: Endpoint reachable but response payload is unexpectedly small"
                        elif service_id == 'scamsniffer':
                            try:
                                payload = response.json()
                                if isinstance(payload, list) and payload:
                                    success = True
                                    message = f"✅ Success: {service['name']} blacklist responded correctly"
                                elif isinstance(payload, dict) and payload:
                                    success = True
                                    message = f"✅ Success: {service['name']} blacklist responded correctly"
                                else:
                                    success = True
                                    message = f"⚠️ {service['name']}: Endpoint reachable but payload is empty"
                            except Exception:
                                success = True
                                message = f"⚠️ {service['name']}: Endpoint reachable but payload parsing failed"
                        elif service_id == 'honeypot':
                            try:
                                payload = response.json()
                                if isinstance(payload, dict) and ('summary' in payload or 'honeypotResult' in payload):
                                    success = True
                                    message = f"✅ Success: {service['name']} responded correctly"
                                else:
                                    success = True
                                    message = f"⚠️ {service['name']}: Endpoint reachable but payload shape changed"
                            except Exception:
                                success = True
                                message = f"⚠️ {service['name']}: Endpoint reachable but payload parsing failed"
                        else:
                        ...
                        (
                            response.status_code in [401, 402, 403, 404, 429, 500, 550]
                            and service_id in ['moralis', 'twitter', 'zapper', 'breadcrumbs', 'reddit', 'dune', 'oklink', 'trmlabs', 'chainabuse', 'honeypot', 'ofac_sls', 'scamsniffer', 'chainalysis_oracle', 'coincap', 'dexscreener', 'lifi', 'birdeye', 'solscan', 'solanatracker']
                        )
                        or (
                            response.status_code in [400, 422]
                            and service_id in ['trmlabs', 'chainabuse', 'chainalysis_oracle', 'honeypot', 'lifi', 'birdeye', 'solscan', 'solanatracker']
                        )
                    ):
                        ...
                        if service_id == 'moralis':
                            ...
                            if response.status_code == 404:
                                ...
                                if 'cloudflare' in body or 'just a moment' in body:
                                    message = f"⚠️ {service['name']}: Request blocked by Cloudflare/WAF (not necessarily an API-key issue)"
                                else:
                                    message = f"⚠️ {service['name']}: Forbidden (Breadcrumbs One is beta/manual-key gated; verify this key is enabled for API One)"
                            ...
                            if response.status_code == 400:
                                message = f"⚠️ {service['name']}: Request rejected (check DUNE_SIM_CHAIN_ID and token address format)"
                            elif response.status_code == 401:
                                message = f"⚠️ {service['name']}: Unauthorized (check DUNE_API_KEY / X-Sim-Api-Key)"
                            elif response.status_code == 403:
                                message = (
                                    f"⚠️ {service['name']}: Access forbidden "
                                    "(key may be valid but this endpoint/plan is restricted)"
                                )
                            elif response.status_code == 404:
                                message = f"⚠️ {service['name']}: Endpoint not found (check DUNE_SIM_BASE_URL)"
                            elif response.status_code == 429:
                                message = f"⚠️ {service['name']}: Rate limited"
                            else:
                                message = f"⚠️ {service['name']}: API reachable but request was rejected ({response.status_code})"
                        elif service_id == 'oklink':
                            ...
                            if response.status_code in [400, 422]:
                                ...
                                message = f"⚠️ {service['name']}: Access issue (public endpoint reachable, or add TRMLABS_API_KEY for higher limits)"
                        elif service_id == 'chainabuse':
                            if response.status_code in [400, 422]:
                                message = f"⚠️ {service['name']}: Endpoint reachable (request parameters rejected)"
                            elif response.status_code == 429:
                                ...
                                message = f"⚠️ {service['name']}: Authentication/authorization issue (check CHAINABUSE_API_KEY)"
                        elif service_id == 'honeypot':
                            if response.status_code in [400, 422]:
                                message = f"⚠️ {service['name']}: Endpoint reachable (address parameter rejected)"
                            elif response.status_code == 429:
                                message = f"⚠️ {service['name']}: Rate limited"
                            else:
                                message = f"⚠️ {service['name']}: Endpoint reachable but request blocked/limited"
                        elif service_id == 'ofac_sls':
                            if response.status_code in [403, 404]:
                                message = f"⚠️ {service['name']}: Dataset host reachable but access path changed"
                            elif response.status_code == 429:
                                message = f"⚠️ {service['name']}: Dataset endpoint rate limited"
                            else:
                                message = f"⚠️ {service['name']}: Dataset endpoint reachable but not returning expected content"
                        elif service_id == 'scamsniffer':
                            if response.status_code == 429:
                                message = f"⚠️ {service['name']}: GitHub raw endpoint rate limited"
                            else:
                                message = f"⚠️ {service['name']}: Blacklist endpoint reachable but response not usable"
                        elif service_id == 'chainalysis_oracle':
                            if response.status_code in [400, 422]:
                                message = f"⚠️ {service['name']}: RPC reachable (eth_call payload rejected)"
                            elif response.status_code == 429:
                                message = f"⚠️ {service['name']}: RPC rate limited"
                            else:
                                message = f"⚠️ {service['name']}: RPC reachable but returned auth/permission error"
                        elif service_id == 'coincap':
                            if response.status_code == 429:
                                message = f"⚠️ {service['name']}: Rate limited / credit throttled"
                            elif response.status_code in [401, 403]:
                                message = f"⚠️ {service['name']}: Authentication issue (check COINCAP_API_KEY)"
                            elif response.status_code == 402:
                                message = f"⚠️ {service['name']}: Credit/quota exhausted for current billing period"
                            else:
                                message = f"⚠️ {service['name']}: Endpoint reachable but request rejected ({response.status_code})"
                        elif service_id == 'lifi':
                            if response.status_code == 429:
                                message = f"⚠️ {service['name']}: Rate limited"
                            elif response.status_code in [401, 403]:
                                ...
                                message = f"⚠️ {service['name']}: Endpoint reachable (request params/token not accepted)"
                            else:
                                message = f"⚠️ {service['name']}: Endpoint reachable but request rejected ({response.status_code})"
                        elif service_id == 'birdeye':
                            if response.status_code == 429:
                                ...
                                message = f"⚠️ {service['name']}: Authentication issue (check BIRDEYE_API_KEY)"
                            elif response.status_code in [400, 404, 422]:
                                message = f"⚠️ {service['name']}: Endpoint reachable (request parameters rejected)"
                            else:
                                message = f"⚠️ {service['name']}: Endpoint reachable but request rejected ({response.status_code})"
                            ...
                            if response.status_code == 429:
                                ...
                                message = f"⚠️ {service['name']}: Authentication issue (check SOLSCAN_API_KEY)"
                            elif response.status_code in [400, 404, 422]:
                                message = f"⚠️ {service['name']}: Endpoint reachable (request parameters rejected)"
                            else:
                                message = f"⚠️ {service['name']}: Endpoint reachable but request rejected ({response.status_code})"
                        ...
                        if hasattr(response, 'text') and response.text:
                            ...
                            if service_id == 'moralis' and 'usage has been consumed' in error_text:
                                ...
                                message = f"⚠️ {service['name']}: API reachable (address/query parameter required)"
                            elif service_id == 'chainabuse' and ('unauthorized' in error_text.lower() or 'forbidden' in error_text.lower()):
                                message = f"⚠️ {service['name']}: Invalid authorization (check CHAINABUSE_API_KEY)"
                            elif service_id == 'honeypot' and ('address' in error_text.lower() and 'invalid' in error_text.lower()):
                                message = f"⚠️ {service['name']}: Endpoint reachable (invalid address supplied)"
                            elif service_id == 'ofac_sls' and ('access denied' in error_text.lower() or 'forbidden' in error_text.lower()):
                                message = f"⚠️ {service['name']}: Dataset host reachable but access denied"
                            elif service_id == 'scamsniffer' and ('not found' in error_text.lower() or '404' in error_text):
                                message = f"⚠️ {service['name']}: Dataset path changed but host reachable"
                            elif service_id == 'chainalysis_oracle' and ('method not found' in error_text.lower() or 'eth_call' in error_text.lower()):
                                message = f"⚠️ {service['name']}: RPC reachable but eth_call payload rejected"
                            elif service_id == 'dune' and ('chain_ids' in error_text.lower() or 'chain id' in error_text.lower()):
                                message = f"⚠️ {service['name']}: Invalid chain_ids for SIM Dune (set DUNE_SIM_CHAIN_ID to a supported chain id)"
                            elif service_id == 'dune' and ('forbidden' in error_text.lower() or '403' in error_text):
                                message = f"⚠️ {service['name']}: Access denied by SIM Dune for this key/plan."
                            elif service_id == 'birdeye' and ('invalid' in error_text.lower() and 'key' in error_text.lower()):
                                message = f"⚠️ {service['name']}: Invalid API key - check BIRDEYE_API_KEY"
                            elif service_id == 'solscan' and ('invalid' in error_text.lower() and 'token' in error_text.lower()):
                                message = f"⚠️ {service['name']}: Invalid API key - check SOLSCAN_API_KEY/SOLSCAN_PRO_API_KEY"
                            elif service_id == 'solanatracker' and ('unauthorized' in error_text.lower() or 'forbidden' in error_text.lower()):
                                message = f"⚠️ {service['name']}: Invalid API key or not yet activated"
                    ...
                    elif service_id == 'dune' and last_request_exception:
                        last_error = str(last_request_exception).lower()
                        if (
                            'failed to resolve' in last_error
                            or 'name or service not known' in last_error
                            ...
                            message = (
                                f"⚠️ {service['name']}: api.sim.dune.com is not resolvable from this environment "
                                "(network/DNS issue, not API-key validation)."
                            )
                        else:
                            message = f"❌ Error: {service['name']} failed to get a response ({last_request_exception})"
                    else:
    ...
    def _update_fetch_result(self, service_id, success, message):
        ...
        button_state = 'normal' if status == 'available' else 'disabled'
        widgets['trigger_btn'].config(text="🔄 Fetch Data", state=button_state)
