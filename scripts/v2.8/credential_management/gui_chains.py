...
print("Running Chain ID Management")
...
class ChainManagementGUI:
    def __init__(self):
        print("🔧 Initializing Chain Management GUI...")
...
Chain IDs are used to identify different blockchain networks for API calls and data fetching."""
        ...
        control_frame.grid(row=3, column=0, sticky="we")
        ...
        commentary_frame.grid(row=4, column=0, sticky="we", pady=(10, 0))
        ...
        for i, comment in enumerate(commentaries):
            ...
            comment_label.grid(row=i, column=0, sticky="w", pady=2)
    ...
    def create_chain_entries(self):
        ...
        for chain_key, chain_info in self.manager.chains.items():
            ...
            chain_frame.grid(row=row, column=0, columnspan=2, sticky="we", pady=(0, 10))
            chain_frame.columnconfigure(1, weight=1)
            
            # Chain ID label
            id_label = ttk.Label(chain_frame, text="Chain ID:", style='Chain.TLabel')
            id_label.grid(row=0, column=0, sticky="w", padx=(0, 10))
            ...
            if chain_info['required']:
                ...
                req_label.grid(row=0, column=2, sticky="w")
            
            # Description
            desc_label = ttk.Label(chain_frame, text=chain_info['description'], style='Desc.TLabel')
            desc_label.grid(row=1, column=0, columnspan=3, sticky="w", pady=(5, 0))
    ...
    def update_widget_theme(self, widget, theme, bg_color, fg_color, entry_bg, entry_fg, label_bg):
        ...
        try:
            ...
            if isinstance(widget, tk.Entry):
                ...
                # ttk widgets use styles, not direct bg/fg
                pass
            # ttk.Canvas does not exist; no special handling needed
...
if __name__ == "__main__":
    print("🚀 Starting chain manager main function...")
    if check_singleton():
