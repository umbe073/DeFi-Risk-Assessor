...
- `DEPLOY_PATH`: Git checkout root on the server.
- `DEPLOY_RUNTIME_TARGET_DIR`: runtime target (often a `scripts/v2.0`-style tree).
- `SOURCE_DIR`: defaults to `${DEPLOY_PATH}/scripts/v2.8`.

Do not set `DEPLOY_RUNTIME_TARGET_DIR` to `DEPLOY_PATH` itself. The runtime target must be the app runtime tree, not the repository root.
